...
* Combined *Query/Search* bar. The project search bar can now be used to build and run a no-code query on a connected graph database. Click for a dropdown query menu to build a query pattern from Category and Relationship elements, and to specify property values. Then click the magnifying glass (search) icon to run the query.
+
...
* *Style Setting* dialog enhancements
...
* Refined bottom toolbar and search bar design, and improved switch handling (GXR-3080, GXR-3101).
...
* Path-finding algorithms added GXR-3125): 
** gxr.dijkstra()
** gxr.yens()
